...
matrix::matrix(int nr , int nc)
{
  ...
  if ( (nr==0)||(nc==0) ) return;
  k=(vector*)calloc( nr , sizeof( vector ) );
  for(i=0;i<nr;i++) k[i]=vector( nc );
  dim=nr;
}


matrix::matrix( int nr ,  const vector* v         ) // Init mit Zeilenvektoren
{
  int i;
  dim=0;k=0;
  if ((nr==0)||(v==0)) return;
...
matrix operator *( const matrix& m1 ,  const matrix& m2)
{
  ...
  matrix local(r1,c2);// components are set to zero

  int i,j,s;
#ifdef MATDEBUG
  int r2=m2.getrows();
  if (c1!=r2) return matrix(0);
  if (!local.is_usable()) return matrix(0);
#endif
  for(i=0;i<r1;i++)
    for(j=0;j<c2;j++)
      for(s=0;s<c1;s++) local[i][j]+=m1[i][s]*m2[s][j]; 
  return matrix(local);
}
...
vector  matrix::getcolvec  (int i)  const
{
#ifdef MATDEBUG
...
int     matrix::setcolvec  (int i ,  const vector& v )
{
#ifdef MATDEBUG
...
int     matrix::rowplus    (int i ,  const vector& v)
{
#ifdef MATDEBUG
...
int     matrix::colplus    (int i ,  const vector& v)
{
#ifdef MATDEBUG
...
int     matrix::rowminus   (int i ,  const vector& v)
{
#ifdef MATDEBUG
  if (   (!is_usable()) || (!v.is_usable()) || (getcols()!=v.dim)
      || (i<0) || (i>=dim) ) return 0;
...
int     matrix::colminus   (int i ,  const vector& v)
{
#ifdef MATDEBUG
  if (   (!is_usable()) || (!v.is_usable()) || (dim!=v.dim)
      || (i<0) || (i>=dim) ) return 0;
...
int     matrix::rp    (int i ,  const vector& v) {k[i]+=v;return 1;}
int     matrix::cp    (int i ,  const vector& v)
{
  int j;for(j=0;j<dim;j++) k[j][i]+=v.k[j];return 1;
}

int     matrix::rm    (int i ,  const vector& v) {k[i]-=v;return 1;}
int     matrix::cm    (int i ,  const vector& v)
{
  int j;for(j=0;j<dim;j++) k[j][i]-=v.k[j];return 1;
}
...
matrix matrix::unity(int d) const // Einheitsmatrix dxd
{
  matrix local(d);
#ifdef MATDEBUG
  if (!local.is_usable()) return matrix(0);
...
scalar	matrix::getDETfromLU	(int *indx,scalar *d) const
{
  int	j;
  scalar	det;
...
void	matrix::backsubst	(int *indx, const vector& b)
{
  int	i,ii=-1,ip,j;
  scalar	sum;
...
scalar	matrix::getDETandINV	(matrix& newinv) const
{
  int	indx[dim];
  scalar	d;
