...
vector::vector( int d      , scalar  s , ... )
{
  ...
  // fprintf(stderr,"This doesn't work on a SUNOD: vector(dim, d1 , d2 , ....ddim )\n");
  // exit(1);
...
vector operator +(  const vector& v1 ,  const vector& v2)
{
  ...
  if (local.k==0) return local; // vector(local);
#endif
...
vector& vector::operator  =(const vector& v)
{
  ...
  if( (NULL!=k) && (v.dim==dim) ) {
    memcpy(k,v.k, dim*sizeof(scalar) );
    return *this;
  }
...
scalar  vector::getabs() const
{
  return sqrt( getsqr() );
}

vector  vector::getuni() const 
{
  vector local;
  ...
  scalar abs;
  abs=getabs();
  if (abs==0.0) return local; //vector(local);
...
int vector::write
(FILE* fp, int dump) const	// dump==0 : don't dump dimension
     // dump!=0 : do it
{
  int i;

  if( dump ) fprintf(fp," %u",dim);
  for(i=0;i<dim;i++) fprintf(fp," %22.20g", k[i]);
  fprintf(fp," \n");
  return 0;
}
